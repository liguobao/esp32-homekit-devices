...
static int _i2c_init(uint8_t port)
{
    ...
    if (ret != ESP_OK) {
        i2c_master_bus_handle_t existing_handle = NULL;
        if (i2c_master_get_bus_handle(port, &existing_handle) == ESP_OK &&
                existing_handle != NULL) {
            i2c_bus_handle[port] = existing_handle;
            i2c_inited[port] = true;
            ESP_LOGI(TAG, "Reuse existing I2C master bus port %d handle %p",
                    port, existing_handle);
            return 0;
        }
